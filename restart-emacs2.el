;;; restart-emacs2.el --- Restart emacs from within emacs  -*- lexical-binding: t; -*-
...
(defgroup restart-emacs2 nil
  "Customization options for restart-emacs2"
  :group 'tools
  :prefix "restart-emacs2-")

(defcustom restart-emacs2-daemon-with-tty-frames-p nil
  "Restart Emacs daemon even if it has tty frames.

Currently `restart-emacs2' cannot restore such frames, it just
notifies the user once the daemon has restarted"
  :type 'boolean
  :group 'restart-emacs2)

(defcustom restart-emacs2-restore-frames nil
...
(defun restart-emacs2--string-join (strings &optional separator)
...
(defun restart-emacs2--user-error (format &rest args)
...
(defvar restart-emacs2--args nil
  "The arguments with which to restart Emacs is bound dynamically.")

(defun restart-emacs2--get-emacs-binary ()
...
(defun restart-emacs2--record-tty-file (current &rest ignored)
...
parameters.  IGNORED are ignored."
  (when (processp (cdr current))
    (let ((window (frame-selected-window (process-get (cdr current) 'frame))))
      (cons 'restart-emacs2-file (buffer-file-name (window-buffer window))))))

(defun restart-emacs2--notify-connection-instructions (tty filename)
...
(defun restart-emacs2--frameset-tty-filter (tty filtered parameters saving)
...
PARAMETERS and SAVING."
  (when (cdr tty)
    (run-at-time 0.5
                 nil
                 (apply-partially 'restart-emacs2--notify-connection-instructions
                                  (cdr tty)
                                  (cdr (assoc 'restart-emacs2-file filtered)))))
  (frameset-filter-tty-to-GUI tty filtered parameters saving))

(defun restart-emacs2--restore-frames-using-desktop (file)
  ...
  (let* (desktop-file-modtime
         ...
         (frameset-filter-alist (append '((tty . restart-emacs2--frameset-tty-filter))
...
(defun restart-emacs2--frame-restore-args ()
  ...
  (when (and (locate-library "frameset")
             ...
             ;; the frames in daemon mode. Also ignore the `restart-emacs2-restore-frames'
             ;; configuration since restarting the daemon without restoring frames
             ;; doesn't really help
             (or (daemonp)
                 (and restart-emacs2-restore-frames
                      (not (bound-and-true-p desktop-save-mode)))))
    (list "--restart-emacs2-desktop"
          (restart-emacs2--save-frames-using-desktop))))

(defun restart-emacs2--start-gui-using-sh (&optional args)
...
ARGS is the list arguments with which Emacs should be started"
  (call-process "sh" nil
                0 nil
                "-c" (format "%s %s &"
                             (shell-quote-argument (restart-emacs2--get-emacs-binary))
                             (restart-emacs2--string-join (mapcar #'shell-quote-argument
                                                                 args)
                                                         " "))))

(defun restart-emacs2--start-gui-on-windows (&optional args)
  "Start GUI version of Emacs on windows.

ARGS is the list arguments with which Emacs should be started"
  (w32-shell-execute "open"
                     (restart-emacs2--get-emacs-binary)
                     (restart-emacs2--string-join args " ")))

(defun restart-emacs2--start-emacs-in-terminal (&optional args)
...
(defun restart-emacs2--daemon-using-sh (&optional args)
...
current instance"
  (call-process "sh" nil
                ...
                "-c" (format "%s --daemon=%s %s &"
                             (shell-quote-argument (restart-emacs2--get-emacs-binary))
                             server-name
                             (restart-emacs2--string-join (mapcar #'shell-quote-argument args)
                                                         " "))))

(defun restart-emacs2--daemon-on-windows (&optional args)
  "Restart Emacs daemon with the provided ARGS.
...
TODO: Not tested yet"
  (w32-shell-execute "open"
                     (restart-emacs2--get-emacs-binary)
                     (restart-emacs2--string-join (cons (concat "--daemon=" server-name)
                                                       args)
                                                 " ")))

(defun restart-emacs2--ensure-can-restart ()
  "Ensure we can restart Emacs on current platform."
  (when (and (not (display-graphic-p))
             (memq system-type '(windows-nt ms-dos)))
    (restart-emacs2--user-error (format "Cannot restart Emacs running in terminal on system of type `%s'" system-type)))

  (when (and (daemonp)
             (not (locate-library "frameset")))
    (restart-emacs2--user-error "Cannot restart Emacs daemon on versions before 24.4"))

  (when (and (daemonp)
             (delq nil (mapcar (lambda (frame)
                                 (frame-parameter frame 'tty))
                               (frame-list)))
             (not restart-emacs2-daemon-with-tty-frames-p)
             (not (yes-or-no-p "Current Emacs daemon has tty frames, `restart-emacs2' cannot restore them, continue anyway? ")))
    (restart-emacs2--user-error "Current Emacs daemon has tty frames, aborting `restart-emacs2'.
Set `restart-emacs2-with-tty-frames-p' to non-nil to restart Emacs irrespective of tty frames")))

(defun restart-emacs2--launch-other-emacs (arguments)
  ...
  (apply (cond ((daemonp) (if (memq system-type '(windows-nt ms-dos))
                              #'restart-emacs2--daemon-on-windows
                            #'restart-emacs2--daemon-using-sh))

               ((display-graphic-p) (if (memq system-type '(windows-nt ms-dos))
                                        #'restart-emacs2--start-gui-on-windows
                                      #'restart-emacs2--start-gui-using-sh))

               (t (if (memq system-type '(windows-nt ms-dos))
                      ;; This should not happen since we check this before triggering a restart
                      (restart-emacs2--user-error "Cannot restart Emacs running in a windows terminal")
                    #'restart-emacs2--start-emacs-in-terminal)))
         ;; Since this function is called in `kill-emacs-hook' it cannot accept
         ;; direct arguments the arguments are let-bound instead
         (list arguments)))

(defun restart-emacs2--translate-prefix-to-args (prefix)
...
(defun restart-emacs2--guess-startup-directory-using-proc ()
...
(defun restart-emacs2--guess-startup-directory-using-lsof ()
...
(defun restart-emacs2--guess-startup-directory-using-buffers ()
...
(defun restart-emacs2--guess-startup-directory-from-env ()
  "Guess the startup directory for current Emacs session from USERPROFILE or HOME."
  (or (getenv "HOME")
      (getenv "USERPROFILE")))

(defun restart-emacs2--guess-startup-directory ()
...
the HOME or USERPROFILE (only applicable on Window) environment variable and
  ...
  (or (restart-emacs2--guess-startup-directory-using-proc)
      (restart-emacs2--guess-startup-directory-using-lsof)
      (restart-emacs2--guess-startup-directory-using-buffers)
      (restart-emacs2--guess-startup-directory-from-env)
      default-directory))
...
;;;###autoload
(defun restart-emacs2-handle-command-line-args (&rest ignored)
  "Handle the --restart-emacs2-desktop command line argument.

The value of the argument is the desktop file from which the frames should be
restored.  IGNORED are ignored."
  (restart-emacs2--restore-frames-using-desktop (pop command-line-args-left)))

;;;###autoload
(add-to-list 'command-switch-alist '("--restart-emacs2-desktop" . restart-emacs2-handle-command-line-args))

(defvar restart-emacs2--inhibit-kill-p nil
  "Non-nil means inhibit killing the current session when restarting.
This means that `restart-emacs2' will spawn a new instance of
Emacs without killing the current one.

This is used internally to implement the command
`restart-emacs2-start-new-emacs'.")

;;;###autoload
(defun restart-emacs2 (&optional args)
  "Restart Emacs.
...
with which Emacs should be restarted."
  ...
  (restart-emacs2--ensure-can-restart)
  ;; We need the new emacs to be spawned after all kill-emacs-hooks
  ;; have been processed and there is nothing interesting left
  (let* ((default-directory (restart-emacs2--guess-startup-directory))
         (translated-args (if (called-interactively-p 'any)
                              (restart-emacs2--translate-prefix-to-args args)
                            args))
         (restart-args (append translated-args
                               ;; When Emacs is started with a -Q
                               ;; restart-emacs2's autoloads would not be present
                               ;; causing the the --restart-emacs2-desktop
         ...
         (kill-emacs-hook (append kill-emacs-hook
                                  (unless restart-emacs2--inhibit-kill-p
                                    (list (apply-partially #'restart-emacs2--launch-other-emacs
                                                           restart-args))))))
    (if restart-emacs2--inhibit-kill-p
        (restart-emacs2--launch-other-emacs restart-args)
        (save-buffers-kill-emacs))))

;;;###autoload
(defun restart-emacs2-start-new-emacs (&optional args)
...
with which the new Emacs should be started."
  (interactive "P")
  (let ((restart-emacs2--inhibit-kill-p t))
    (restart-emacs2 args)))

(provide 'restart-emacs2)
;;; restart-emacs2.el ends here
